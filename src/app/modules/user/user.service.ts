...
export const createUserDB = async (payload: TUser) => {
  ...
  const hashPassword = await bcrypt.hash(payload.password, Number(config.SALT_ROUND) as number);
...
export const updateUserDB = async (user: TJwtDecode, payload: Partial<User>) => {
...
export const deleteUserDB = async (userId: string, user: TJwtDecode) => {
  ...
  if (userInfo.email === user.email) {
    throw new CustomError(StatusCodes.BAD_REQUEST, "You can't modify yourself!");
  }
