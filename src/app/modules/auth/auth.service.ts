...
export const loginUserDB = async (payload: { email: string; password: string }) => {
  ...
  const comparePassword = await bcrypt.compare(payload.password, isUserExist.password);

  if (!comparePassword) {
    throw new CustomError(StatusCodes.NOT_FOUND, 'Please, Enter correct password!');
  }
...
export const changePasswordDB = async (
  ...
  const hashPassword = await bcrypt.hash(payload.newPassword, Number(config.SALT_ROUND) as number);
